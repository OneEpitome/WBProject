  ...
  const { errors, handleChange, handleSubmit } = useForm({
    ...
    validate: ({ username, password, confirmPassword, nickname }) => {
      ...
      if (username && username.length < 5)
        errors.usernameLength = "아이디는 5자 이상이여야 합니다!";
      ...
      if (password && !passwordRegex.test(password))
        errors.passwordRegex =
          "숫자, 영문, 특수문자 조합 8자 이상으로 입력해주세요!";

      // 비밀번호 확인 유효성 검사
      if (!confirmPassword)
        errors.confirmPassword = "비밀번호 재확인은 필수정보입니다!";
      if (confirmPassword && password !== confirmPassword)
        errors.notMatchPassword = "비밀번호가 일치하지 않습니다!";
